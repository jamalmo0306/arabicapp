...
AppProvider (src/context/app-context.tsx)
  └─ ThemeProvider + Stack (src/app/_layout.tsx)
       ...
       │    ├─ log.tsx           Activity log + line chart + delete
       │    ├─ music.tsx         Arabic song lyrics viewer (Arabic/Latin/EN modes)
...
- `colors` (AppColors) — reactive palette derived from `settings.dark_mode`; `isDark` (boolean)
...
Key actions: `logSession`, `saveFlashcardBatch`, `saveCheckIn`, `patchSettings`, `triggerConfetti`, `refreshStats`, `markCard(id, status)`, `importWeeklyCards(weekNumber, rawCards)`, `dismissWeekComplete`, `clearWeekCards(weekNumber)`, `deleteLog(id)`.
...
### Styling system — dark mode

All main screens use a single theming pattern:
...
`makeStyles(C: AppColors)` is a module-level function that returns `StyleSheet.create({...})`. Styles recompute when the theme changes. Sub-components in the same file receive `C` and/or `s` as props (or call `useAppContext()` directly when they have their own separate StyleSheet, as `SongDetail` and `AddEditModal` do in `music.tsx`).

The two palettes live in `src/lib/theme.ts` — `LIGHT` (sand desert) and `DARK` (night desert). `getColors(mode)` returns the right one. `AppColors` is the shared type. Key tokens:

| Token | Light | Dark |
|---|---|---|
| `scrollBg` / `safe` | `#CBB77C` (sand) | `#1A1409` (dark desert) |
| `blackGlass` | `rgba(14,15,15,0.88)` | `rgba(8,6,2,0.94)` |
| `textDark` | `#2C251C` | `#EDD9A3` |
| `heroGradTop/Bot` | `#CBB77C` | `#1A1409` |
| `statusBar` | `'dark-content'` | `'light-content'` |

The old pattern (`src/constants/theme.ts`, `ThemedView`, `Colors.light/dark`, `Spacing`) still exists in legacy UI components but is not used by any main screen.
...
The card has a fixed height (`screenHeight * 0.46` via `useWindowDimensions`). The front face shows a **Show Answer** button; rating buttons only appear after the card is flipped. Flipping toggles a local `flipped` state with no animation.
...
**Shuffle:** `restart()` and the `reviewWeek` archive load both apply Fisher-Yates shuffle before setting the queue. Initial load of current-week cards is in import order.
...
| `songs` | Arabic song lyrics (title, artist, arabic_lyrics, transliteration, english_translation) |
...
- `deleteActivityLog(id)` — removes a single log entry; AppContext `deleteLog(id)` calls this then refreshes `weekActivityDates`
...
`logSession` in AppContext calls `insertActivityLog` when `activityType` is provided, then refreshes `weekActivityDates` from the DB.
...
- Time-range selector (1W / 1M / 3M / 6M / 1Y); chart y-axis shows **minutes** (not count)
- Pure-View line chart (no SVG library) — line segments computed via midpoint + rotation transform
- Session history list with a `✕` delete button per row; tapping calls `deleteLog(id)` which removes the entry and refreshes `weekActivityDates` (streak dots update if that was the only activity on that date)

### Music screen

`src/app/(tabs)/music.tsx` stores Arabic song lyrics in the `songs` table. Each song has three aligned text fields — Arabic lyrics, transliteration, and English translation — stored as newline-separated strings. The detail view splits all three by `\n` and renders them line-by-line so each lyric line shows its Arabic / transliteration / English stacked together.

View mode toggle: **All** / **عربي** / **Latin** / **EN** — hides the other layers.

A **Get Prompt** button in the header opens a modal containing a ready-to-use Claude prompt. Users copy this prompt, paste it into Claude web (replacing song name/artist), then paste the three returned sections into the Add Song form. The modal has a **Share** button using the native `Share` API.
...
The native tab bar is hidden. Navigation is handled by `src/components/bottom-nav.tsx`, a floating pill overlay rendered in `(tabs)/_layout.tsx`. `BottomNav` uses `useSegments()` to determine the active tab and `router.push()` to navigate. Six tabs: Home, Cards, Log, Music, Library, Settings.
...
- `src/lib/db.web.ts` — SQLite no-ops plus full in-memory implementations for `flashcard_archive`, `activity_log`, and `songs` (all CRUD). Data resets on page refresh — expected behaviour.
...
1. **Phrase of the Day card** — full-width card; parses `settings.phrase_of_day` (JSON `PhraseOfDay`), falls back to a card from `currentWeekCards`. Uses `C.cardLight` for background (adapts to dark mode).
2. **Hero** — 400px full-bleed desert image (`marginHorizontal: -16`) with camel sprite and `LinearGradient` overlays fading to `C.heroGradTop` / `C.heroGradBot` (sand in light, dark desert in dark mode).
3. **Quick-launch row** — Duolingo / LingQ / Weekly Resource as three dark-glass cards. Weekly Resource title/subtitle/URL are editable in Settings. Duolingo/LingQ use `Linking.openURL(scheme://).catch(() => openURL(https://...))` fallback.
